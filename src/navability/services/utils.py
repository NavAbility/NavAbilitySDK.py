...
def waitForCompletion(
    ...
    Args:
        ...
        expectedStatus (str, optional): Expected status message per request. Defaults to "Complete".
    """
    wait_time = maxSeconds
    while any(
        [
            getStatusLatest(navAbilityClient, res).state != expectedStatus for res in requestIds
        ]
    ):
        time.sleep(2)
        wait_time -= 2
