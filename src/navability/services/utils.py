...
def GraphVizApp(client, variableStartsWith=None):
    ...
    try:
        return md(
            f"""[![Navigate to Factor Graph](http://www.navability.io/wp-content/uploads/2022/03/factor_graph.png)]({topography_vis_link})"""
        )
    except:
        return
...
def MapVizApp(client, variableStartsWith=None):
    ...
    try:
        return md(
            f"""[![Navigate to Factor Graph](http://www.navability.io/wp-content/uploads/2022/03/geometric_map.png)]({geometry_vis_link})"""
        )
    except:
        return
